...
describe("GET /api/tickets", () => {
  it("should return status 200 and all tickets submitted by the user", async () => {
    // given
...
describe("GET /api/tickets/:id", () => {
  ...
  it("should return status 403 when the ticket is outside the user's company", async () => {
    // given
    const { token } = await createUser()
    const company = await Company.create({ name: "Other Company" })
    const user = await User.create({ sub: "5678", name: "Other User", email: "other@test.com", company: company._id })
    const ticket = await Ticket.create({ createdBy: user._id, subject: "Private ticket", company: company._id })
    // when
    const response = await request(app)
      .get(`/api/tickets/${ticket._id}`)
      .set("Authorization", `Bearer ${token}`)
...
describe("PUT /api/tickets/:id", () => {
  ...
  it("should return status 403 when updating another user's ticket", async () => {
    // given
    const { token } = await createUser()
    const company = await Company.create({ name: "Other Company" })
    const user = await User.create({ sub: "5678", name: "Other User", email: "other@test.com", company: company._id })
    const ticket = await Ticket.create({ createdBy: user._id, subject: "Subject", company: company._id })
    // when
    const response = await request(app)
      .put(`/api/tickets/${ticket._id}`)
      .set("Authorization", `Bearer ${token}`)
