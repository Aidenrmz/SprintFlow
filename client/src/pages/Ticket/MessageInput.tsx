...
const MessageInput = ({ refresh }: Props) => {
  ...
  const sendNewMessage = async () => {
    ...
    if (!id || !user || trimmedMessage.length < 1) return toast.error("Write your message before sending.")
    const response = await sendMessage(id, { user: user._id, message: trimmedMessage })
