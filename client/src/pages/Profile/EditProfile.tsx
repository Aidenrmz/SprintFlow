...
const EditProfile = ({ user, refresh }: Props) => {
  ...
  const updateUserProfile = async () => {
    if (!updateData.name || !updateData.email) return toast.error("Please fill all required fields before saving.")
    if (!emailRegex.test(updateData.email)) return toast.error("Please add a valid email address.")
